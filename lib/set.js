...
module.exports = class Set {
    ...
    async process() {
        ...
        let halves = [];
        console.log(this.set.createHalfPlates);
    ...
    async getInfo(plates) {
        let info = [];
        for (const plate of plates) {
            const body = await this.moonraker.getInfo(config.uploadToFolder+"/"+this.set.name+"_"+plate.fileOnly+".gcode");
