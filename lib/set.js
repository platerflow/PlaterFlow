...
module.exports = class Set {
    ...
    async process() {
        ...
        if ( this.config.uploadToFolder != undefined ) {
            ...
            this.log("done uploading all plates");
    ...
    async slicePlates(plates, baseColor) {
        for ( const plate of plates ) {
            this.log("slicing plate " + plate.file);
    ...
    async getInfo(plates) {
        let info = [];
        for (const plate of plates) {
            const body = await this.moonraker.getInfo(this.config.uploadToFolder+"/"+this.set.name+"_"+plate.fileOnly+".gcode");
